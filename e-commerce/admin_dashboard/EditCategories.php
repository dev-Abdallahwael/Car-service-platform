...
if(isset($_SESSION['user_id'])&& isset($_GET['id'])){
    ...
    if(mysqli_num_rows($runQuerySelect) == 1){
        $category = mysqli_fetch_assoc($runQuerySelect);
        $_SESSION['category'] = "found";
    }else{
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                ...
                <div class="card p-4 shadow-sm mt-4">
                    ...
                    <form action="handle/EditCategoryHandle.php?id=<?php echo $category['id']; ?>" method="post" >
                        <div class="mb-3">
                            <label for="category" class="form-label">Category Name</label>
                            <input type="text" class="form-control" id="category" value="<?php echo $category['name']; ?>" name="category" required>
                        </div>
                        <div class="mb-3">
                            <label for="descr" class="form-label">Description</label>
                            <textarea class="form-control" id="descr" name="descr" rows="3" required> <?php echo $category['descr']; ?></textarea>
                        </div>
                        <button class="btn btn-primary" type="submit" name="submit">Edit Category</button>
                    </form>
                </div>
            </main>
        </div>
