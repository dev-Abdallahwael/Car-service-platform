...
if(isset($_GET['id'])){
    ...
    if(mysqli_num_rows($runQuery) == 1 ){
        ...
        $_SESSION['productScan'] = "found";
       
    }else{
        $_SESSION['productScan'] = "not found";
    }

    $queryCategory = "SELECT id, name from categories";
    $runQueryCategory = mysqli_query($conn,$queryCategory);
    if(mysqli_num_rows($runQueryCategory)>0){
        $categories = mysqli_fetch_all($runQueryCategory,MYSQLI_ASSOC);
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                ...
                <h2>
                  <?php
                //   session_start();
                    if(isset($_SESSION['productScan'])){
                      echo $_SESSION['productScan'];
                    }
                ...
                <div class="card p-4 shadow-sm mt-4">
                    ...
                    <form action="handle/productEditHandle.php?id=<?php echo $product['id']; ?>" method="post" enctype="multipart/form-data">
                        <div class="mb-3">
                            <label for="category" class="form-label">Product Name:</label>
                            <input type="text" class="form-control" id="category" value="<?php echo $product['name'];?>" name="category" required>
                        </div>
                        <div class="mb-3">
                            <label for="number" class="form-label">Price:</label>
                            <input type="text" class="form-control" id="price" value="<?php echo $product['price'];?>" name="price" required>
                        </div>
                        <div class="mb-3">
                            <label for="number" class="form-label">Discount:</label>
                            <input type="number" class="form-control" id="discount" value="<?php echo $product['discount'];?>" name="discount" required>
                        </div>
                        <div class="mb-3">
                            <label for="descr" class="form-label">Description:</label>
                            <textarea class="form-control" id="descr" name="descr"  rows="3" required><?php echo $product['descr'];?></textarea>
                        </div>
                        <div class="mb-3">
                        ...
                        <img src="../../uploads/<?php echo $product['image']; ?>" alt="" width="100px">
